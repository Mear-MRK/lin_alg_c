 ...
 * vec - Vector type.
 *
 * This struct represents a vector with elements of type FLD_TYP.
 * It contains:
...
/*
 * Allocates `d` FLD_TYP for v->pyl->arr.
 * `d` must be non-zero;
...
FLD_TYP vec_dot(const vec *v_left, const vec *v_right);
...
/* NOT implemented yet.
FLD_TYP vec_mean(const vec* v);
FLD_TYP vec_std(const vec* v);
*/
